...
public class FrmRelatorioAplicacao extends FrmRelatorioPai {
	...
	public FrmRelatorioAplicacao() {
		setTitle("Relatório - Aplicações");
		setSize(getWidth()+60, getHeight());
		lblTitulo.setText("Relatório de Aplicações");
	...
	public void confirmar() {
		...
		if (rBtnLavoura.isSelected()) {
			rel.relatorioAplicacoes("lavoura.lav_numero");
		} else if (rBtnDataAplicacao.isSelected()) {
			rel.relatorioAplicacoes("aplicacao.apl_data");
		} else if (rBtnInsumo.isSelected()) {
			rel.relatorioAplicacoes("insumo.ins_descricao");
		} else if (rBtnDataFinal.isSelected()) {
			rel.relatorioAplicacoes("data_final");
		}
